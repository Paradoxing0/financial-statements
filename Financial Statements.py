...
def store(y):
    ...
    g = StatementOfEquity
    h = BalanceSheet
    arcyears[y] = [h.Assets.mixlist,f.revenue,f.cogs,f.salaries_exp,f.sell_admin_exp,f.interest_exp,f.tax_rate,
...
def ISPrint():
    f = IncomeStatement
    print("Income Statement [Year %s]" % (year))
...
def BSPrint():
    ...
    def ppeind():
        for i in f.Assets.mixlist:
            print("      P.P.E. #%s:" % (f.Assets.mixlist.index(i)+1))
            print("         Total Value: %s" % (i[0]))
            print("         Total Depreciation: %s" % (i[1]))
            print("         Depreciaiton Rate: %s/yr" % (i[2]))

    print("Balance Sheet [Year %s]" % (year))
...
def SEPrint():
    f = StatementOfEquity
    g = IncomeStatement
    print("Statement of Equity [Year %s]" % (year))
...
def CFPrint():
    ...
    print("Statement of Cash Flow [Year %s]" % (year))
    print("Net Income: %s" % f.net_income(f))
...
def new():
                ...
                while ppenum > 0:
                    ...
                    while True:
                        try:
                            ...
                            print("There appears to be an error inputting accumulated depreciation. Try again.")
                    while True:
                        try:
                            c = float(input("What is the rate of depreciation per year?:"))
                            break
                        except ValueError:
                            print("There appears to be an error inputting rate of depreciation. Try again.")
                    ppenum -= 1
                    n += 1
                    ppelist.append(a)
                    deplist.append(b)
    ...
    if h.balanced(h) == False:
        ...
        while True:
            ...
            if i == "change":
                x = input("Choose which account to change.")
                new_adj(x)
                calc_cash_0()
                if h.balanced(h) == False:
...
print("Welcome to the Financial Statement Simulation. This simulation is a work-in-progress, and meant to be used"
      " for educational purposes.\n \nMade by Freddy U.\n")

while True:
    start = input("Would you like to use a sample set or start a new one from stractch? (WARNING: choosing to start a"
                  "new one from stratch will be lengthy and difficult for those with little to no"
                  " knowledge/experience with accounting:\n")
    if (start not in ("new","New")) is False:
        new()
        break
    elif (start not in ("sample","Sample")) is False:
        sample()
        break
    else:
...
while True:
    ...
    if (option not in ("print","Print")) is False:
        ...
        print("'print': this option provides the option to display one of the four financial statements on the "
              "screen. Note: the term 'print' does not actually print out information on paper, but displays "
              "information on the prompt.")
        print("'journal entry': this option does nothing. This will soon be used as a way to create journal entries "
              "in the upcoming future.")
    elif (option not in ("journal entry")) is False:
        pass
    else:
